...
async def agente_arraigo_socioformativo(state: dict) -> dict:
    llm  = get_llm("arraigo_socioformativo")
    ...
    rag    = buscar_contexto(pregunta, tramite="arraigo_socioformativo")
    ...
    prompt = ARRAIGO_SOCIOFORMATIVO_PROMPT.format(
        bloque_cultural=bloque,
        especialidad="arraigo para la formación (socioformativo)",
        contexto_rag=rag,
    )
    ...
    return {
        ...
        "tramite_detectado": "arraigo_socioformativo",
        "last_agent":        "arraigo_socioformativo",
